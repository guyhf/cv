...
sub handle_file {
    ...
    $outfile = print_html_file($texfilename);
    return $outfile
}
...
sub guy_postfix {
  ...
  my $cont = <F>;
  close F;
  $cont =~ s/Unnamed Web page/Guy Haskin Fernald/;
  $cont =~ s/<BODY>/<BODY BGCOLOR="F0F0F0" TEXTCOLOR="0F0F0F">/;

  $cont =~ s/<BIG><I><B><B>(.*)<\/B><\/B><\/I><\/BIG><HR>/<TABLE WIDTH=100%><TR><TD><BIG><I><B><B>\1<\/B><\/B><\/I><\/BIG><\/TD><TD ALIGN=RIGHT><FORM ACTION="guyhf.pdf" METHOD="GET"><INPUT TYPE="SUBMIT" VALUE="Print"><\/INPUT><\/FORM><\/TD><TR><TD COLSPAN=2><HR><DIV ALIGN=RIGHT><\/TD><\/TABLE>/;


  $cont =~ s#\n<TD ALIGN=RIGHT></TD></TR>\n#\n#g;
  $cont =~ s#</EM>&nbsp;<STRONG>(\d\d\d\d)</STRONG></TD>\n#</EM></TD><TD ALIGN=RIGHT><STRONG>\1</STRONG></TD>\n#g;

  $cont =~ s/\\\~/\~/g;
  $cont =~ s/\\sim/~/g;
  $cont =~ s/\\'//g;
  $cont =~ s#<TR><TD>&nbsp;#<TD ALIGN=RIGHT>&nbsp;#g;
  $cont =~ s/\n<HR>\n.*<\/ADDRESS>//ms;
